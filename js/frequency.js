...
let csvFile = "";
const newLine = '\r\n';
let fields = ['Date', 'Frequency (Hz)', 'Magnitude (dB)', 'SPRKPOS (mVrms)', 'Input Level (Vrms)'];
    ...
    if(document.getElementById("freqExcelPath").value != ""){ //a csv is actually specified
        fs.stat(document.getElementById("freqExcelPath").value, function (err, stat) {
            if (err == null) {
                ...
                console.log("file: " + document.getElementById("freqExcelPath").value)
...
ipcRenderer.on("freq point", (event, arg) =>{ //display on label what signal is generated
    ...
    date = new Date();
    writeCSV([date, arg["x"], arg["y"], arg["sprkmvrms"], arg["inputLevel"]]);
